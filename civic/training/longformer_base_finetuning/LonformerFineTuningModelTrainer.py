...
class LongFormerFineTuningModelTrainer(IModelTrainer):
    ...
    def do_model_training(self, n_epochs):
        scheduler = scheduler = LambdaLR(self.optimizer, lr_lambda=lr_lambda)
            ...
            for e in range(1, n_epochs):
                ...
                for batch in self.train_data_loader:
                    train_loss += self.batch_training_step.train_batch(
                        batch, self.model, self.optimizer, scheduler
                    )
                    idx += 1
                    print(
